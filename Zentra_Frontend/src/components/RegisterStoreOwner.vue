<template>
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12 d-flex justify-content-between align-items-center p-3 bg-light shadow-sm">
                <div class="d-flex align-items-center">
                    ...
                    <h4 class="mb-0">Zentras - Store</h4>
                </div>
        ...
        <div class="row vh-100 bg-newColor">
            <div class="col-12 col-md-7 d-flex flex-column justify-content-center align-items-center text-white">
            ...
            <div class="col-12 col-md-5 d-flex align-items-center justify-content-center">
                <div class="border rounded p-4 shadow bg-light" style="width: 100%; max-width: 400px;">
                    <h5 class="mb-3">Đăng ký Store</h5>
        ...
        <div v-if="showOTPModal" class="modal-overlay">
            <div class="modal-box">
                ...
                <div v-if="!otpVerified">
                    ...
                    <button v-if="canResendOTP" class="btn btn-link text-danger p-0" @click="resendOTP">Chưa nhận được
                        mã OTP? Gửi lại</button>
        ...
        <!-- Modal nhập thông tin cửa hàng -->
        <div v-if="showStoreInfoForm" class="modal-overlay">
            <div class="modal-box" style="max-width: 600px;">
                <div class="d-flex mb-4">
                    <div class="step" :class="{ 'active': currentStep >= 1 }">Thông tin Shop</div>
                    <div class="step" :class="{ 'active': currentStep >= 2 }">Cài đặt vận chuyển</div>
                    <div class="step" :class="{ 'active': currentStep >= 3 }">Thông tin thuế</div>
                    <div class="step" :class="{ 'active': currentStep >= 4 }">Thông tin định danh</div>
                    <div class="step" :class="{ 'active': currentStep >= 5 }">Hoàn tất</div>
                </div>
                ...
                <div v-if="currentStep === 1">
                    ...
                    <div class="mb-3">
                        ...
                        <input v-model="storeName" class="form-control" placeholder="Nhập tên shop" />
                        <div class="text-end text-muted small">10/30</div>
                        <p v-if="storeNameError" class="text-danger small">{{ storeNameError }}</p>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">*Địa chỉ lấy hàng</label>
                        <button class="btn btn-outline-secondary btn-sm"
                            @click="showAddressDetailForm = !showAddressDetailForm">+ Thêm</button>
                        <p v-if="addressError" class="text-danger small">{{ addressError }}</p>
                    </div>

                    <div v-if="showAddressDetailForm" class="mb-3">
                        <label class="form-label">*Chi tiết địa chỉ</label>
                        <input v-model="addressDetail" class="form-control" placeholder="Số nhà, đường, phường/xã" />
                        <p v-if="addressDetailError" class="text-danger small">{{ addressDetailError }}</p>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">*Email</label>
                        <input v-model="storeEmail" type="email" class="form-control" placeholder="Nhập email" />
                        <p v-if="storeEmailError" class="text-danger small">{{ storeEmailError }}</p>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">*Số điện thoại</label>
                        <input v-model="storePhone" type="tel" class="form-control" placeholder="Nhập số điện thoại" />
                        <p v-if="storePhoneError" class="text-danger small">{{ storePhoneError }}</p>
                    </div>

                    <div class="d-flex justify-content-between mt-4">
                        <button class="btn btn-outline-secondary" @click="showStoreInfoForm = false">Huỷ</button>
                        <button class="btn bg-newColorbtn" @click="nextStep">Tiếp theo</button>
                    </div>
                </div>

                <!-- Bước 2: Địa chỉ lấy hàng chi tiết -->
                <div v-if="currentStep === 2">
                    <h5 class="mb-3">Địa chỉ lấy hàng</h5>

                    <div class="mb-3">
                        <label class="form-label">*Họ tên</label>
                        <input v-model="addressName" class="form-control" placeholder="Nhập họ tên" />
                        <p v-if="addressNameError" class="text-danger small">{{ addressNameError }}</p>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">*Số điện thoại</label>
                        <input v-model="addressPhone" type="tel" class="form-control"
                            placeholder="Nhập số điện thoại" />
                        <p v-if="addressPhoneError" class="text-danger small">{{ addressPhoneError }}</p>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">*Địa chỉ</label>
                        <input v-model="address" class="form-control" placeholder="Nhập địa chỉ" />
                        <p v-if="addressError" class="text-danger small">{{ addressError }}</p>
                    </div>
                    ...
                    <div class="d-flex justify-content-between mt-4">
                        <button class="btn btn-outline-secondary" @click="prevStep">Quay lại</button>
                        <button class="btn bg-newColorbtn" @click="nextStep">Tiếp theo</button>
                    </div>
                </div>
                ...
                <div v-if="currentStep === 5">
                    ...
                    <p>Bạn đã hoàn thành tất cả các bước đăng ký cửa hàng.</p>

                    <div class="d-flex justify-content-between mt-4">
                        <button class="btn btn-outline-secondary" @click="prevStep">Quay lại</button>
                        <button class="btn bg-newColorbtn" @click="completeRegistration">Hoàn tất</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    methods: {
        ...
        nextStep() {
            if (this.currentStep === 1) {
                ...
                if (!this.storeName) {
                    this.storeNameError = "Vui lòng nhập tên shop";
                    return;
                }
                if (!this.storeEmail) {
                    this.storeEmailError = "Vui lòng nhập email";
                    return;
                }
                if (!this.storePhone) {
                    this.storePhoneError = "Vui lòng nhập số điện thoại";
                    return;
                }
            }
            else if (this.currentStep === 2) {
                this.addressNameError = '';
                this.addressPhoneError = '';
                this.addressError = '';
                this.addressDetailError = '';

                if (!this.addressName) {
                    this.addressNameError = "Vui lòng nhập họ tên";
                    return;
                }
                if (!this.addressPhone) {
                    this.addressPhoneError = "Vui lòng nhập số điện thoại";
                    return;
                }
                if (!this.address) {
                ...
                if (!this.addressDetail) {
                    this.addressDetailError = "Vui lòng nhập chi tiết địa chỉ";
                    return;
                }
            }
